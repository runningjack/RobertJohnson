...
class Country{
	...
	protected static $db_fields=array('country_id','name','iso_code_2','iso_code_3','address_format','postcode_required','status');
	public $country_id;
	public $name;
	public $iso_code_2;
	public $iso_code_3;
	...
	public static function find_by_id($id){
		global $database;
		$result_array =self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE country_id=".$id);
	...
	public function create(){
		...
		if ($database->db_query($sql)){
			$this->country_id = $database->insert_id();
			return true;
		}
		else{
			return false;
	...
	public function update() {
	  global $database;
		...
		$sql .= " WHERE country_id=". $this->country_id;
	  $database->db_query($sql);
	  return ($database->affected_rows() == 1) ? true : false;
	}
	public function delete(){
		global $database;
		$sql = "DELETE FROM ".self::$table_name." WHERE country_id=".$this->country_id;
		$database->db_query($sql);
		return ($database->affected_rows() == 1) ? true : false;
	}
}
