...
class Slide {
	...
	protected static $db_fields=array('s_id','s_link','s_path','s_show', 's_name');
	public $s_id;
	public $s_link;
	public $s_path;
	public $s_show = "hide";
	...
	public static function find_by_id($id){
		global $database;
		$result_array =self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE s_id=".$id);
		return !empty($result_array) ? array_shift($result_array) : false;
	}
	
	public static function get_slides(){
		global $database;
		$result_set =self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE s_show = 'show' ");
		return $result_set;
	}
	...
	public function update() {
	  global $database;
		...
		$sql .= " WHERE s_id=". $this->s_id;
	  $database->db_query($sql);
	  return ($database->affected_rows() == 1) ? true : false;
	}
	public function delete(){
		global $database;
		$sql = "DELETE FROM ".self::$table_name." WHERE s_id=".$this->s_id;
		$database->db_query($sql);
		return ($database->affected_rows() == 1) ? true : false;
	}
