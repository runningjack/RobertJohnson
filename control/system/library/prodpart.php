<?php
class Prodpart{
	protected static $table_name="tbl_part_replace";
	protected static $db_fields=array('id','item_id','works_id','prod_id','part_name','qty','unit_cost','total_cost','remarks','datecreated');
	public $id;
    public $item_id;
    public $works_id;
    public $prod_id;
    ...
    public static function find_by_WSheet($id) {
		global $database;
		$result_set = $database->db_query("SELECT * FROM ".self::$table_name." WHERE works_id=".$id);
		$object_array = array();
		while ($row = $database->fetch_array($result_set)) {
		  $object_array[] = self::instantiate($row);
		}
